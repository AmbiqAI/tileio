...
const Device = types
  ...
  .views(self => ({
    ...
    get recordDurationStr(): string {
      ...
      return self.record.durationHMS;
    },
  ...
  .actions(self => ({
    receivedSlotData: flow(function*(slot: number, signals: number[][], mask: number[][]) {
      ...
      if (self.record) { yield self.record.addSlotData(slot, signals, mask); }
    }),
    receivedSlotMetrics: flow(function*(slot: number, metrics: number[]) {
      if (slot >= self.slots.length) { return; }
      self.slots[slot].addMetrics([metrics]);
      if (self.record) { yield self.record.addSlotMetrics(slot, [metrics]); }
  ...
  .actions(self => ({
    ...
    setInfo: flow(function*(info: IDeviceInfoSnapshot) {
